...
**Frontend has since moved to Blazor (C#), and mobile to .NET MAUI Blazor Hybrid (C#)** — see `CLAUDE.md`'s "Stack split" section for the current, authoritative record; the rest of this README predates that migration and still describes the original React/Next.js/Ionic stack in places.
...
│   └── mobile/
│       └── customer-mobile/        # Mobile app (.NET MAUI Blazor Hybrid, C#) — see .claude/rules/mobile.md, least-verified part of the migration
