...
public sealed class WebhookDeliveryService
{
    ...
    public async Task PublishEventAsync(string eventType, IReadOnlyDictionary<string, object?> data, CancellationToken cancellationToken = default)
    {
        ...
        {
            _db.WebhookDeliveries.Add(new Entities.WebhookDelivery
            {
    ...
    private async Task DeliverWebhookAsync(Entities.WebhookDelivery delivery, CancellationToken cancellationToken)
    {
        ...
        var signature = WebhookSignatureService.GenerateSignature(payloadString, delivery.Subscription.Secret);
