...
public sealed class TokenService
{
    ...
    public TokenPair GenerateToken(string userId, string username, string roleName, bool rememberMe)
    {
        ...
        var accessToken = CreateToken(
            userId, username, roleName, permissions, TokenScope.Customer, accessTokenId, "access", _accessKey, AccessTokenExpiry);
    ...
    public async Task LogoutAsync(string userId, string? accessToken, string? refreshToken)
    {
        ...
        if (accessToken is not null)
        {
            ...
            {
                tasks.Add(BlacklistTokenAsync(accessPayload.Jti, 3600));
            }
        }
