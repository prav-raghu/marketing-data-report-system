...
public sealed class TokenService
{
    ...
    private static readonly TimeSpan DefaultAccessTokenExpiry = TimeSpan.FromHours(1);
    private static readonly TimeSpan DefaultRefreshTokenShortExpiry = TimeSpan.FromDays(1);
    ...
    private readonly TimeSpan _refreshTokenShortExpiry;
    ...
    public TokenService(CustomerApiOptions options, IConnectionMultiplexer redis)
    {
        ...
        _accessTokenExpiry = DurationParser.Parse(options.AuthTokenExpiry, DefaultAccessTokenExpiry);
        _refreshTokenShortExpiry = DurationParser.Parse(options.RefreshTokenExpiry, DefaultRefreshTokenShortExpiry);
        _redis = redis;
    }

    private IDatabase? Database => _redis.IsConnected ? _redis.GetDatabase() : null;

    public TokenPair GenerateToken(string userId, string username, string roleName, bool rememberMe)
    {
        ...
        var accessToken = CreateToken(
            userId, username, roleName, permissions, TokenScope.Customer, accessTokenId, "access", _accessKey, _accessTokenExpiry);

        var refreshExpiry = rememberMe ? RefreshTokenLongExpiry : _refreshTokenShortExpiry;
        var refreshToken = CreateToken(
            userId, username, roleName, permissions, TokenScope.Customer, refreshTokenId, "refresh", _refreshKey, refreshExpiry);

        _ = StoreRefreshTokenAsync(userId, refreshTokenId, rememberMe);
    ...
    private TokenPayload? Verify(string token, SymmetricSecurityKey key, string expectedType)
    {
        try
        {
            ...
            var iat = jwt.Payload.TryGetValue("iat", out var iatValue) && iatValue is not null
                ? Convert.ToInt64(iatValue, CultureInfo.InvariantCulture)
                : (long?)null;
            ...
            return new TokenPayload
            {
                ...
                Role = principal.FindFirstValue("role") ?? principal.FindFirstValue(ClaimTypes.Role) ?? string.Empty,
    ...
    public async Task<TokenPair?> RefreshTokenAsync(string token, bool rememberMe)
    {
        ...
        var newTokens = GenerateToken(payload.Id, payload.Username, payload.Role, rememberMe);
    ...
    public async Task LogoutAsync(string userId, string? accessToken, string? refreshToken)
    {
        ...
        if (accessToken is not null)
        {
            ...
            {
                tasks.Add(BlacklistTokenAsync(accessPayload.Jti, (int)_accessTokenExpiry.TotalSeconds));
            }
        }
    ...
    public async Task InvalidateAllAccessTokensAsync(string userId)
    {
        ...
        try
        {
            ...
            await db.StringSetAsync($"{MinIatPrefix}{userId}", nowSeconds.ToString(CultureInfo.InvariantCulture), _accessTokenExpiry);
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> IsSessionInvalidatedAsync(string userId, long? issuedAt)
    {
        ...
        try
        {
            ...
            return long.TryParse((string?)minIat, out var minIatValue) && issuedAt < minIatValue;
        }
        catch (Exception ex)
        {
    ...
    private int GetRefreshTokenTtl(bool rememberMe) => rememberMe ? LongSessionTtlSeconds : (int)_refreshTokenShortExpiry.TotalSeconds;
}
