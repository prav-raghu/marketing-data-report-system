...
public sealed class WebhookProcessorJob
{
    ...
    private async Task DeliverWebhookAsync(AppDbContext db, WebhookDelivery delivery, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var signature = _signatureService.GenerateSignature(payloadJson, delivery.Subscription.Secret);
    ...
    private async Task HandleFailedDeliveryAsync(AppDbContext db, WebhookDelivery delivery, CancellationToken cancellationToken)
    {
