...
public sealed class ApiKeyMiddleware
{
    ...
    private readonly ScheduleApiOptions _options;

    public ApiKeyMiddleware(RequestDelegate next, ScheduleApiOptions options)
    {
        _next = next;
        _options = options;
    }
    ...
    {
        if (key is null || key.Length != _options.ScheduleApiKey.Length)
        {
            return false;
        }
        return CryptographicOperations.FixedTimeEquals(
            Encoding.UTF8.GetBytes(key),
            Encoding.UTF8.GetBytes(_options.ScheduleApiKey));
    }
