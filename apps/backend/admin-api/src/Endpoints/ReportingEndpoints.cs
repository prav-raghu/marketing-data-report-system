    ...
    public static void MapReportingEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/generate", async (GenerateReportDto body, ReportingService reportingService) =>
        {
            var request = new ReportRequest
            {
                ...
                Filters = body.Filters is null
                    ...
                    : new ReportFilter
                    {
                        StartDate = body.Filters.StartDate is { } start ? DateTime.Parse(start, CultureInfo.InvariantCulture).ToUniversalTime() : null,
                        EndDate = body.Filters.EndDate is { } end ? DateTime.Parse(end, CultureInfo.InvariantCulture).ToUniversalTime() : null,
            ...
            string? endDate) =>
        {
            ...
            var request = new ReportRequest
            {
                ...
                {
                    StartDate = startDate is not null ? DateTime.Parse(startDate, CultureInfo.InvariantCulture).ToUniversalTime() : null,
                    EndDate = endDate is not null ? DateTime.Parse(endDate, CultureInfo.InvariantCulture).ToUniversalTime() : null,
                },
                IncludeHeaders = true,
            };
