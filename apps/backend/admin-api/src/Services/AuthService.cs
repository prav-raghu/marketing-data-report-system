...
public sealed class AuthService
{
    ...
    public async Task<LoginResponseDto> VerifyLoginMfaAsync(VerifyLoginMfaRequestDto model, CancellationToken cancellationToken = default)
    {
        ...
        if (db is null)
        {
            return new LoginResponseDto { IsSuccessful = false, Message = "Verification temporarily unavailable. Please try again shortly." };
        }

        var lockCount = await db.StringGetAsync(lockKey);
        if (lockCount.HasValue && int.TryParse((string?)lockCount, out var attempts) && attempts >= MfaMaxAttempts)
        {
    ...
    public async Task<ResetPasswordResponseDto> ResetPasswordAsync(ResetPasswordRequestDto model, CancellationToken cancellationToken = default)
    {
        ...
        user.Password = BCrypt.Net.BCrypt.HashPassword(model.Password, workFactor: PasswordHashing.WorkFactor);
        user.AuthHash = null;
