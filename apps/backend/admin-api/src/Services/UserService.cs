...
public sealed class UserService
{
    ...
    public async Task<UserProfileDto?> GetUserProfileAsync(string userId, CancellationToken cancellationToken = default)
    {
        ...
        if (user is not null)
        {
            await _db.Entry(user).Reference(u => u.Roles).LoadAsync(cancellationToken);
            await _db.Entry(user).Reference(u => u.Status).LoadAsync(cancellationToken);
        }

        if (user?.Roles is null || !RoleName.AdminTierRoles.Contains(user.Roles.Name))
        {
            return null;
        }
        ...
        return new UserProfileDto
        {
            ...
            Roles = new RoleSummaryDto { Id = user.Roles.Id, Name = user.Roles.Name },
            Status = user.Status != null ? new StatusSummaryDto { Id = user.Status.Id, Name = user.Status.Name } : null,
        };
    }
    ...
    public async Task<OnboardingResponseDto> OnboardUserAsync(OnboardingRequestDto model, CancellationToken cancellationToken = default)
    {
        ...
        await _emailService.SendMailAsync(
            ...
            {
                ["username"] = user.Username,
                ["dateLoggedIn"] = DateTime.UtcNow.ToString("G", CultureInfo.InvariantCulture),
        ...
        string userId, string currentPassword, string newPassword, CancellationToken cancellationToken = default)
    {
        ...
        user.Password = BCrypt.Net.BCrypt.HashPassword(newPassword, workFactor: PasswordHashing.WorkFactor);
        await _db.SaveChangesAsync(cancellationToken);
