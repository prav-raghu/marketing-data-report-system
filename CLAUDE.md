...
| `mobile` | Ionic + Capacitor customer-mobile app |
...
| `typescript-standards` | Frontend/mobile type-safety review outside a full code review — no `any`, no `as unknown as T`, no `@ts-ignore` (unaffected by the backend migration; scoped to `apps/frontend/*`/`apps/mobile/*` now that the backend has its own `csharp-standards`) |
...
These skills predate the .NET migration and are frontend-focused (React/Next.js), so they're unaffected by the backend stack change.
...
### Frontend (TypeScript — unchanged by the migration)

- React (admin-web, Vite SPA) and Next.js (customer-web) — no other frontend framework
- Ionic + Capacitor for `customer-mobile`
- TypeScript strict — no `any`, no `as unknown as T`, no `@ts-ignore`
- Zod for frontend form/client-side validation
...
- Before marking any TypeScript task complete, run `pnpm --filter <app> tsc --noEmit` — zero errors required
- Write frontend unit tests where feasible

## Folder structure (immutable)

```text
apps/backend/        ASP.NET Core (.NET 10) Minimal API services
apps/frontend/       React (admin-web) and Next.js (customer-web) — unchanged, still TypeScript
apps/mobile/         Ionic + Capacitor — unchanged, still TypeScript
...
- **Frontend/mobile (TypeScript)**: pnpm — always use `pnpm`, never `npm` or `yarn`, for anything under `apps/frontend/*` or `apps/mobile/*`. Internal deps use `workspace:*`.
...
| customer-api | 4002 |
| schedule-api | 4003 |
| customer-web | 3000 (Next.js) |
| admin-web | 80 (nginx, Traefik-routed in production); dev server defaults to 4004 (`VITE_ADMIN_PORT`, see `vite.config.ts`) |
| customer-mobile (Vite dev) | 5173 / 5174 (Vite defaults, no explicit `VITE_MOBILE_PORT` override) |
| cms | 4005 |
| automation (workflow-api) | 4006 |

All four backend services are ASP.NET Core now; the ports are unchanged from the Node era so nothing downstream (gateway routing, frontend API base URLs, Coolify config) needed to move. `cms`/`automation` are new port assignments, not carried over — Strapi and n8n used their own default ports (1337, 5678) which never needed to match this table.

## Using this as a template — project name substitution

When forking for a new project, replace `node-mono-repo-template` with the project slug everywhere, and `@node-mono-repo-template/` with `@your-scope/` for the remaining frontend/mobile npm packages. For the backend, replace the `DotNetMonoRepoTemplate` C# namespace/library prefix (`DotNetMonoRepoTemplate.Types`, `DotNetMonoRepoTemplate.Database`, etc., and the `DotNetMonoRepoTemplate.sln` filename itself) with your project's PascalCase name. This covers `package.json` name fields, `tsconfig.base.json` path aliases, `pnpm-workspace.yaml`, import paths, every `.csproj`/`.sln` file, `CLAUDE.md`, every file under `.claude/agents/`, every file under `.claude/commands/`, and every file under `.claude/instructions/`.
...
- **Phone validation regex** `^(\+27|0)[6-8][0-9]{8}$` — appears in `api-builder.md` (now as a FluentValidation `.Matches()` rule), `frontend-forms` content in `rules/frontend.md`, `validation-chain.instructions.md`, and `build-page/SKILL.md`. Swap this for the target country's phone format before generating any forms.
