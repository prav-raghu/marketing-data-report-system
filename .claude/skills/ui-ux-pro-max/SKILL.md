...
This skill provides professional design guidance for this monorepo's frontend stack: React + Vite (admin-web), Next.js (customer-web), all using Tailwind v4. `customer-mobile` is .NET MAUI Blazor Hybrid (C#, since Phase 5) — this skill's `--stack` flags below don't cover that stack; use the Tailwind v4 token/design-principle guidance in this file directly for MAUI Blazor Hybrid work, not a `--stack` search.
...
| customer-mobile (.NET MAUI Blazor Hybrid) | no `--stack` flag covers this — use the token/design-principle guidance in this file directly |
