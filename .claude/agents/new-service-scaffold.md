...
apps/frontend/     React (admin-web) / Next.js (customer-web) — pnpm, unchanged
...
Backend: C# namespace/library prefix `DotNetMonoRepoTemplate.` for every `common/*` library, referenced via `<ProjectReference>` (never a NuGet feed for internal libraries). Frontend/mobile: npm scope `@node-mono-repo-template/` for `apps/frontend/*`/`apps/mobile/*`, `workspace:*` internal deps. Never mix — a backend library never gets an npm scope, a frontend package never gets a C# namespace.
