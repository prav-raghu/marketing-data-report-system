...
public sealed partial class RedisCacheService
{
    ...
    [LoggerMessage(Level = LogLevel.Warning, Message = "Redis operation failed, using fallback")]
    private static partial void LogRedisOperationFailed(ILogger logger, Exception exception);
    ...
    private async Task<T> SafeExecuteAsync<T>(Func<IDatabase, Task<T>> operation, T fallback)
    {
        ...
        {
            LogRedisOperationFailed(_logger, ex);
            return fallback;
        }
    }
    ...
    public Task<UserPresenceCacheEntry?> GetUserPresenceAsync(string userId) =>
        SafeExecuteAsync(
            async db =>
            {
                ...
                return value.HasValue ? JsonSerializer.Deserialize<UserPresenceCacheEntry>((string)value!) : null;
            },
    ...
    public Task<IReadOnlyList<T>?> GetCachedConversationsAsync<T>(string userId) =>
        SafeExecuteAsync(
            async db =>
            {
                ...
                return value.HasValue ? JsonSerializer.Deserialize<IReadOnlyList<T>>((string)value!) : null;
            },
