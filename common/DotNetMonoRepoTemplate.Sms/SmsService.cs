...
public sealed partial class SmsService : ISmsService
{
    ...
    public async Task<bool> SendSmsAsync(string recipient, string message, CancellationToken cancellationToken = default)
    {
        if (!_options.Enabled)
        {
            _logger.Info("SMS disabled - skipping", new Dictionary<string, object?> { ["to"] = recipient });
            return false;
        }
        ...
        var destination = ToE164(recipient);
        if (destination is null)
        {
            _logger.Error("Invalid phone number - cannot convert to E164", new Dictionary<string, object?> { ["to"] = recipient });
            return false;
        }
